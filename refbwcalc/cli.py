...
from refbwcalc import DEFAULT_COMPARISON_BW, Bandwidth, Cost, calculate_cost
...
def output_terminal(refbw, comp_list, color):
    ...
    print(
        "Calulating the OSPF cost values"
    ...
    for comp in generate_calc_list(refbw, comp_list):
        ...
        print(
            "Cost at {c.bright_green}{r.compbw.value:>5,.4g}"
...
def main(argv=sys.argv[1:]):
    ...
    parser.add_argument(
        "ref_bw", metavar="REFBW", help="Reference bandwidth to calulate against"
    )
    ...
    parser.add_argument(
        ...
        metavar="BW",
        nargs="*",
        default=DEFAULT_COMPARISON_BW,
        dest="bw_values",
        help="Bandwith values to test (default: %s)" % ", ".join(default_compairisons),
    )
    parser.add_argument(
        "-a",
        "--add-comparison",
